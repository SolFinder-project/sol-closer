/**
 * Pump user-fee PDA reclaim with fee and referral.
 * Uses buildClosePumpPdaInstruction from pumpClose.ts (requires PUMP_CLOSE_DISCRIMINATOR set).
 * Isolated from prod until instruction format is confirmed – see docs/PUMP-CLOSE-DEV-PLAN.md.
 */
...
import { buildClosePumpPdaInstruction, isPumpCloseAvailable } from './pumpClose';
...
/**
 * Close Pump user-fee PDAs and pay fee + referral (same model as empty/dust reclaim).
 * Throws if PUMP_CLOSE_DISCRIMINATOR is not set (instruction format not validated).
 */
export async function closePumpPdas(
  pdas: PumpPdaAccount[],
  ...
  if (!isPumpCloseAvailable()) {
    throw new Error(
      'Pump close instruction not available. Set PUMP_CLOSE_DISCRIMINATOR in lib/solana/pumpClose.ts after validating format (see docs/PUMP-CLOSE-DEV-PLAN.md).'
    );
  }
  ...
  try {
    ...
    if (pdas.length === 0) {
      throw new Error('No Pump PDAs to close');
    }
    ...
    try {
      ...
      logger.error('Pump reclaim Supabase save error:', supabaseError);
    }
    ...
    logger.error('closePumpPdas error:', message);
    return {
      signature: '',
      accountsClosed: 0,
      solReclaimed: 0,
